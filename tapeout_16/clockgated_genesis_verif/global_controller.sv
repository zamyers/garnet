...
//	
//
//  Source file: /nobackup/kzf/garnet/global_controller/genesis/global_controller.svp
...
always @ (posedge clk or posedge reset_in) begin
    if (reset_in==1) begin
        ...
        if (state==ready) begin
            ...
            read_stall: begin
                if (axi_activated)  gc_to_axi_rd_data <= {31'b0, stall};
                else                config_data_jtag_in <= {31'b0, stall};
                read <= 0;
                write <= 0;
                glb_write <= 0;
                glb_read <= 0;
